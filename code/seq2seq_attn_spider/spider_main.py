        ...
        for _ in range(max_decode_len):
            ...
            for beam in beams:
                ...
                for idx, log_prob in enumerate(pred[-1]):
                    if idx in data_loader.vocabs_f_by_db[db]:
...
def test():
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    encoder = Encoder(len(data_loader.vocabs_q), 512, 2).to(device)
    decoder = Decoder(len(data_loader.vocabs_q), 512, 2, len(data_loader.vocabs_f)).to(
        device
    )
    encoder.load_state_dict(torch.load("encoder.pth", map_location=device))
    decoder.load_state_dict(torch.load("decoder.pth", map_location=device))

    test_data = data_loader.load_data("seq2seq_geoqueries/test.txt")
    max_decode_len = max([len(logic) for _, logic in test_data])

    for i, (lang, logic) in enumerate(test_data):
        lang = lang.to(device)
        logic = logic.to(device)

        h, c = encoder(lang)
        outputs = torch.tensor(
            [data_loader.get_start_idx() for _ in range(max_decode_len + 1)]
        ).to(device)
        for j in range(max_decode_len):
            pred, h, c = decoder(outputs[: j + 1], h, c)
            out = pred.max(1)[1][-1]
            outputs[j + 1] = out.item()
            if out.item() == data_loader.get_end_idx():
                break
            print(data_loader.vocabs_f[out.item()], end=" ")
        print()
